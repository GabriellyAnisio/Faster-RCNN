...
# Caminhos para seus arquivos
yolo_labels_path = "Test\Annotations-1"  # Pasta onde estão os arquivos .txt de anotação YOLO
images_path = r"images\test"  # Pasta onde estão as imagens correspondentes
output_voc_path = "Test\Annotations-1-xml"  # Onde salvar os arquivos XML
classes = ["0", "1", "2"]  # Lista de classes do seu dataset
...
def yolo_to_voc(yolo_file, image_file):
    ...
    with open(yolo_file, "r") as file:
        for line in file:
            ...
            # Converter coordenadas YOLO para VOC
            ...
            ET.SubElement(obj, "name").text = classes[class_id]  # Nome da classe
...
# Rodar o script para todos os arquivos de anotação
for txt_file in os.listdir(yolo_labels_path):
    if txt_file.endswith(".txt"):
        yolo_path = os.path.join(yolo_labels_path, txt_file)
        image_path = os.path.join(images_path, txt_file.replace(".txt", ".png"))  # Ajuste a extensão conforme necessário

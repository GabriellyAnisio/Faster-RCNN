...
def compute_metrics2(det_boxes, gt_boxes, iou_thresholds=[0.5], method='area'):
    """
    Compute Precision, Recall, mAP50, and mAP50-95.
    
    Args:
        det_boxes (list): List of detected boxes for each image.
        gt_boxes (list): List of ground truth boxes for each image.
        iou_thresholds (list): List of IoU thresholds for mAP calculation.
        method (str): Method for computing AP ('area' or 'interp').
    
    Returns:
        precision (float): Precision at the specified IoU threshold.
        recall (float): Recall at the specified IoU threshold.
        map50 (float): mAP at IoU threshold 0.5.
        map50_95 (float): mAP averaged over IoU thresholds from 0.5 to 0.95.
    """
    gt_labels = {cls_key for im_gt in gt_boxes for cls_key in im_gt.keys()}
    gt_labels = sorted(gt_labels)
    ...
    for iou_threshold in iou_thresholds:
        ...
        for idx, label in enumerate(gt_labels):
            ...
            # If no ground truth boxes, skip this class
            if num_gts == 0:
                all_aps[label] = np.nan
                continue
        ...
        # Compute mean AP for this IoU threshold
        mean_ap = np.nanmean(aps) if len(aps) > 0 else np.nan
        ...
        if iou_threshold == 0.5:
            ...
            map50_95_list.append(mean_ap)
        
        # Compute precision and recall at this IoU threshold
    ...
    # Compute mAP50-95 as the average of mAP over all IoU thresholds
    map50_95 = np.nanmean(map50_95_list) if len(map50_95_list) > 0 else np.nan
    
    # Return metrics
    return precision_list[0], recall_list[0], map50, map50_95

def compute_metrics(det_boxes, gt_boxes, iou_thresholds=[0.5], method='area'):
    """
    Compute Precision, Recall, mAP50, and mAP50-95.
    ...
    for iou_threshold in iou_thresholds:
        ...
        for idx, label in enumerate(gt_labels):
            ...
            # Store precision and recall for this class
            class_precisions.append(np.nanmean(precisions))
            class_recalls.append(np.nanmean(recalls))
        ...
        # Compute precision and recall at this IoU threshold
        precision = np.nanmean(class_precisions)
        recall = np.nanmean(class_recalls)
        precision_list.append(precision)
        recall_list.append(recall)
...
def evaluate_map(args):
    ...
    # Compute metrics
    precision, recall, map50, map50_95 = compute_metrics(preds, gts, iou_thresholds=[0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95], method='interp')
